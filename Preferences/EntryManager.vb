Module EntryManager
    ...
    Public Function ToEntry(application As String, name As String, value As String) As Entry
        ...
        Dim entryType As EntryType = entryType.Undefined
        If Double.TryParse(value, number) Then
            ...
            End If
            If Not entryType = entryType.Number Then Throw New NotSupportedException("Profile entry type """ & internalName & """ is not registered as a number, but value passed was """ & number & """")
            Return New Entry(application, internalName, number)
        Else
            value = value.Trim(" ", vbTab, """").ToLower
            If Not _EntryTypes.TryGetValue(internalName, entryType) Then
                _EntryTypes(internalName) = entryType.Enumeration
                entryType = entryType.Enumeration
            End If
            If Not entryType = entryType.Enumeration Then Throw New NotSupportedException("Profile entry type """ & internalName & """ is not registered as a enumeration, but value passed was """ & value & """")
